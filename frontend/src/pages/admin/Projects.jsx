...
const Projects = () => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (editMode) {
        await adminApi.put(`/projects/${currentId}`, payload);
      } else {
  ...
  const filteredProjects = projects.filter(p => {
    ...
    const matchesCategory = selectedCategoryId === 'ALL' || p.categoryId === parseInt(selectedCategoryId);
    ...
    <div className="w-full">
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h3 className="font-poppins font-extrabold text-2xl text-primary">Projects Portfolio</h3>
            <p className="text-xs text-on-surface-variant font-medium">Manage showcase case studies, customer procurements, and design completions.</p>
          </div>
          <button 
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border border-outline-variant/20">
            ...
            <span className="text-xs text-on-surface-variant font-bold mt-2">Loading projects portfolio...</span>
          </div>
          ...
          <div className="text-center py-20 bg-white rounded-2xl border border-outline-variant/20">
            ...
            <p className="text-xs text-on-surface-variant font-medium mt-2">No projects found. Add one to get started!</p>
          </div>
        ) : (
            ...
            {filteredProjects.map((proj) => (
              ...
              >
                ...
                <div className="p-5 flex-grow flex flex-col justify-between">
                  <div>
                    <span className="text-[10px] font-bold text-gold-accent bg-gold-accent/10 px-2 py-0.5 rounded uppercase tracking-wider">
                      {categories.find(c => c.id === proj.categoryId)?.name || 'General'}
                    </span>
                    <h4 className="font-poppins font-bold text-base text-primary mt-2">{proj.name}</h4>
                    <p className="text-xs text-on-surface-variant line-clamp-3 mt-1.5">{proj.description || 'No description provided.'}</p>
                  </div>
                  <div className="mt-4 pt-3 border-t border-outline-variant/20 flex justify-between items-center text-[10px] font-bold text-on-surface-variant/80">
                    <span>Client: {proj.clientName || 'N/A'}</span>
                    <span>Date: {proj.completionDate || 'N/A'}</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        {showModal && (
          <div className="fixed inset-0 bg-[#000d22]/70 flex items-center justify-center p-4 z-[999] overflow-y-auto">
            <motion.div 
              ...
              className="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden border-t-4 border-gold-accent"
            >
              <div className="p-6 border-b border-outline-variant/20 flex justify-between items-center bg-[#000d22]/5">
                <h4 className="font-poppins font-extrabold text-base text-primary">
                  {editMode ? 'Edit Showcase Project' : 'Add New Showcase Project'}
                </h4>
              ...
              <form onSubmit={handleSave} className="p-6 space-y-4 text-xs">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="font-label-md text-[10px] text-on-surface-variant font-bold uppercase block mb-1">Project Name</label>
                    <input 
                      required
                      type="text"
                      value={formData.name}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="font-label-md text-[10px] text-on-surface-variant font-bold uppercase block mb-1">Category</label>
                    <select 
                      required
                  ...
                  <div>
                    <label className="font-label-md text-[10px] text-on-surface-variant font-bold uppercase block mb-1">Completion Date</label>
                    <input 
                      required
                      type="date"
                ...
                <div>
                  <label className="font-label-md text-[10px] text-on-surface-variant font-bold uppercase block mb-1">Project Image</label>
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    placeholder="Provide details about the procurement scope, materials, and quantities delivered..."
                    className="w-full rounded-lg border-outline-variant focus:border-gold-accent focus:ring-1 focus:ring-gold-accent p-3 text-body-md bg-white"
                  />
                </div>
